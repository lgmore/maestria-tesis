...
Si se considera una imagen digital discreta en escala de grises $I$, sea $n_i$ el número de ocurrencias del nivel de gris $i$. La probabilidad de ocurrencia de un pixel de nivel $i$ dentro de la imagen digital está dada por:

\begin{equation}
    p_x(i)= p(x=i)=\frac{n_i}{n}, \qquad 0 \leq i < L
\end{equation}

Donde $L$ es el número total de niveles de gris representables dentro de la imagen, $n$ es el total de pixeles de la imagen, y $p_x(i)$ es el histograma de valor $i$ para el pixel, normalizado entre $[0,1]$.

También, definamos la \textit{Función de distribución acumulativa} correspondiente a $p_x$ como:
